...
def make_parser():
    ...
    parser.add_argument(
        "--path", default="rtsp://18770748650:19990728c@192.168.180.72:8554/streaming/live/1", help="path to images or video"
    ...
    parser.add_argument(
        "--save_name",
        type=str,
        default="test_tank_helicopter",
        help="save name for results txt/video",
    )

    parser.add_argument("-c", "--ckpt", default="weights/best_yolov5s6_low_multiscale.pt", type=str, help="ckpt for eval")
    parser.add_argument(
        "--device",
        default="0",
        type=str,
    ...
    parser.add_argument("--conf_thres", default=0.2, type=float, help="test conf")
    parser.add_argument("--iou_thres", default=0.8, type=float, help="test nms threshold")
    parser.add_argument("--tsize", default=(960, 960), type=tuple, help="test image size")
    parser.add_argument(
        "--fp16",
        dest="fp16",
        default=False,
...
def imageflow_demo(predictor, vis_folder, current_time, args):
    ...
    #     save_path, cv2.VideoWriter_fourcc(*"XVID"), fps, (int(height), int(width))
    # )

        # 是否显示标签
        hide_labels = False
        hide_conf = False
        # names = ['round_pavilion', 'car', 'hydrological_station','tank', 'helicopter', 'umbralla', 'pavilion', 'rubber_boat', 'life_buoy']
        names = ['tingzi', 'che', 'shuiwenzhanfang','tanke', 'zhishengji', 'yusan', 'tingzi', 'xiangpiting', 'jiushengquan']
        line_thickness = 3
        save_conf = True
        res_file = os.path.join(vis_folder, f"{save_name}.txt")
    ...
    # while True:
        ...
        im_vis = im0.copy()
        annotator = Annotator(im_vis, line_width=line_thickness, example=str(names))
        ...
        if outputs[0] is not None:
            ...
            for *xyxy, conf, cls in reversed(outputs[0]):
                if args.save_result:  # Write to file
            ...
            timer.toc()
            annotator.show_fps(len(outputs[0]), frame_id=frame_id + 1, fps=1. / timer.average_time)
        else:
            timer.toc()
            online_im = img_info['raw_img']
        online_im = annotator.result()
        # if args.save_result:
        #     # online_im = cv2.resize(online_im,(960, 960))
...
if __name__ == "__main__":
    ...
    assert args.tsize in [(1280, 1280), (640, 640), (980, 980), (800, 800), (960, 960)], "tsize not supported, only (800, 1440) and (608, 1088)"
    main(args)